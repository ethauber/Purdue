...
static constexpr int C=4; // number of characeters per word
...
public:
	...
	Key subset_subt(const std::vector<Key>&, bool verbose=false);
};
...
Key& Key::operator-=(const Key& other) {
    ...
    for (int i = C-1; i >= 0; --i) {
       t = m_digit[i];	
       m_digit[i] = (t - other.m_digit[i] - carry + 32)  % R; 
       carry      = (t - other.m_digit[i] - carry + 32) < R; 
    }
	return *this;
}
...
Key Key::subset_subt(const std::vector<Key>& T, bool verbose) {
	if (T.size() != N) 
		throw std::invalid_argument("dictionary has wrong size");
	Key sum; 
	for (int i = 0; i < N; ++i) {
...
inline bool Key::equal(const Key& other) const{
	if (m_digit != other.m_digit) {
		return false;	
	}
	return true;
}
